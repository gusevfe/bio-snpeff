...
STDERR.puts u.inspect 
      expect(u.inspect).to eq(%{[{:effect=>{:type=>{:name=>"synonymous_variant"@0, :qualifier=>nil}, :attributes=>{:effect_impact=>"LOW"@19, :functional_class=>"SILENT"@23, :codon_change=>"acG/acA"@30, :amino_acid_change=>"p.Thr37Thr/c.111G>A"@38, :amino_acid_length=>"73"@58, :gene_name=>"KCNQ1"@61, :transcript_biotype=>"protein_coding"@67, :gene_coding=>"CODING"@82, :transript_id=>"ENST00000380776"@89, :exon_rank=>"2"@105, :genotype_number=>"1"@107, :errors=>[]}}}]})
    end

    it "should parse errors" do 
      u = parser.effects.parse("splice_region_variant(LOW|||c.25A>G|192|KCNQ1|protein_coding|CODING|ENST00000496887|2|1|WARNING_TRANSCRIPT_INCOMPLETE)") 
      expect(u.inspect).to eq(%{[{:effect=>{:type=>{:name=>"splice_region_variant"@0, :qualifier=>nil}, :attributes=>{:effect_impact=>"LOW"@22, :functional_class=>[], :codon_change=>[], :amino_acid_change=>"c.25A>G"@28, :amino_acid_length=>"192"@36, :gene_name=>"KCNQ1"@40, :transcript_biotype=>"protein_coding"@46, :gene_coding=>"CODING"@61, :transript_id=>"ENST00000496887"@68, :exon_rank=>"2"@84, :genotype_number=>"1"@86, :errors=>[{:error=>"WARNING_TRANSCRIPT_INCOMPLETE"@88}]}}}]})
    end
  end
end
